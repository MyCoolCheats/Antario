...
void ESP::DrawSounds()
{
	for (unsigned int i = 0; i < footsteps.size(); i++)
	{
		...
		playerColor.a = min(powf(percent * 2, 0.6f), 1.f) * playerColor.a; // fades out alpha when its below 0.5
...
{
	if (Settings::ESP::enabled && Settings::NoScopeBorder::enabled && strcmp("HudZoom", pPanel->GetName(vgui_panel)) == 0)
		return false;
...
{
	if (Settings::ESP::Sounds::enabled && pSample && pEngine->IsInGame())
