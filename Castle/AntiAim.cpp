...
bool AntiAim::GetBestHeadAngle(Vector& angle)
{
	...
	for (float a = 0; a < (M_PI * 2.0); a += step)
	{
		...
		pTrace->TraceRay(ray, 0x4600400B, &traceFilter, &tr);
...
void DoAntiAimY(Vector& angle, int command_number, bool bFlip, bool& clamp)
{
	...
	case AntiAimType_Y::ANGEL_SPIN:
		clamp = false;
		factor = (pGlobalVars->curtime * 5000.0f);
...
void DoAntiAimX(Vector& angle, bool bFlip, bool& clamp)
{
